/**
 * MCP Server factory for Deno Edge Functions.
 * Uses factory pattern with context injection instead of AsyncLocalStorage.
 */
...
export function createMcpServer(getContext: () => HandlerContext) {
  ...
  server.widget(
    ...
    (input) => handleListDecks(input as Record<string, never>, getContext())
  );

  // Tool for searching decks by language, difficulty, and category
  server.tool(
    "searchDeck",
    ...
    (input) => handleSearchDeck(input, getContext())
  );
  ...
  server.tool(
    ...
    (input) => handleSaveDeck(input, getContext())
  );
  ...
  server.widget(
    ...
    (input) => handleStartStudySessionFromScratch(input, getContext())
  );
